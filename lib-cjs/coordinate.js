...
})(Direct = exports.Direct || (exports.Direct = {}));
/**
 * 根据目标点获取相对于原点的角度
 ...
 */
function getAngle(origin, target, direct = Direct.top) {
    const [x1, y1] = origin;
    const [x2, y2] = target;
 ...
 * @param rotate
 */
function getRotatePoint(center, radius, rotate) {
 ...
 */
function twoBezier(t, startPos, endPos, controlPoint) {
 ...
 */
function getBorderWidthByCos(a, b, angle) {
    // 角度化弧度
    const C = angle * Math.PI / 180;
    const c2 = a ** 2 + b ** 2 - 2 * a * b * Math.cos(C);
    return Math.sqrt(c2);
}
exports.getBorderWidthByCos = getBorderWidthByCos;
/**
 ...
 */
function getBorderWidthBySin(a, angleA, angleB) {
    // 角度化弧度
...
exports.getBorderWidthBySin = getBorderWidthBySin;
