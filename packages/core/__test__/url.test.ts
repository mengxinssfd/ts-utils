...
test('urlParse', () => {
  ...
  expect(u.getUrlPath('/index.php/admin/MonitorResultManager')).toBe(
...
test('queryStringify', () => {
  ...
  expect(u.stringifyUrlSearch({ a: '1123', b: [1, 2], d: { d: 1, e: 2 } })).toBe(
    'a=1123&b[0]=1&b[1]=2&d[d]=1&d[e]=2',
  );
  expect(u.stringifyUrlSearch({ a: '1123', b: [1, 2], d: { d: 1, e: undefined } })).toBe(
    'a=1123&b[0]=1&b[1]=2&d[d]=1',
  );
...
test('getUrlParam', () => {
  ...
  expect(fn('pname', url, true)).toEqual(
...
test('updateUrlParam', () => {
  ...
  expect(fn({ id: url }, url)).toEqual(
    `https://www.test.com/Openapi/api_detail?id=${encoded}#api-parameter`,
  );
  expect(fn({ id: url }, url, false)).toEqual(
...
test('UrlRegExp', () => {
  ...
  expect(
    'http://test.com,https://hello.cn'.match(new RegExp(`${subString(ure.source, 1, -1)}`, 'g')),
...
test('isUrl', () => {
  ...
  // 不能识别普通网址的端口号
  expect(
    fn('http://www.baidu.com:112332/index.php/admin/MonitorResultManager/monitorData'),
  ).toBeFalsy();
  ...
  // 端口号范围在2位数到5位数
  expect(
    fn('http://localhost:111222/index.php/admin/MonitorResultManager/monitorData'),
  ).toBeFalsy();
  ...
  // 识别hash
  expect(
    fn('http://www.baidu.com/index.php/?a%5B%5D=123&a%5B%5D=on&b%5B0%5D=on#api-parameter'),
  ).toBeTruthy();
  // 识别[]
